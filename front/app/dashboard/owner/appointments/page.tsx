...
import { Calendar, Clock, Search, Plus, Filter, ChevronLeft, ChevronRight, MoreHorizontal, Trash2, Edit, X, CheckCircle, AlertCircle } from 'lucide-react'
import { toast, Toaster } from "sonner"
import type { User, AuthResponse, Appointment, Service, UserAppointment } from "@/app/api/types"
...
export default function OwnerAppointments() {
  ...
  const filteredAppointments = hasAppointments(userData)
    ? userData.appointments.filter((appointment: Appointment) => {
        ...
        const clientNames = appointment.Users.map((u: UserAppointment) => 
          `${u.firstname} ${u.lastname}`.toLowerCase()).join(" ");
        
        // Try to find service name
        let serviceName = "";
        if (hasServices(userData)) {
          const service = userData.services.find((s: Service) => s.id === appointment.service);
          if (service) {
        ...
        const matchesStatus = statusFilter === "all" || statusFilter === appointmentStatus;
  ...
  const getServiceName = (serviceId: string): string => {
    ...
    return serviceCache[serviceId] ? serviceCache[serviceId]["service-name"] : "Loading...";
  };
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-white/70">Manage your client appointments</p>
          </div>
          <Link
                ...
                <tbody>
                  {currentAppointments.map((appointment: Appointment) => (
                    <tr key={appointment.ID} className="border-b border-white/5 hover:bg-white/5">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          {appointment.Users.map((user: UserAppointment, index: number) => (
                            <div key={index}>
                              <p className="font-medium">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <p className="font-medium">{getServiceName(appointment.service)}</p>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
