...
const SkeletonFallback = (props: any) => <div className="bg-white/10 animate-pulse" {...props} />
...
export default function ProvidersPage() {
  ...
  const [filteredProviders, setFilteredProviders] = useState<OwnerWithServices[]>([])
  ...
  useEffect(() => {
    ...
    const filtered = providers.filter((provider) => {
      ...
      const matchesService =
        selectedService === "all" ||
        provider.services?.some((service: Service) => service["service-name"] === selectedService)
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="space-y-6">
        <div className="flex flex-col space-y-2">
          <h1 className="text-3xl font-bold tracking-tight text-white">Service Providers</h1>
          <p className="text-white/70">Find the perfect service provider for your pet's needs</p>
        </div>
        ...
        {/* Service Filter Tabs */}
        <Tabs defaultValue="all" value={selectedService} onValueChange={setSelectedService} className="w-full">
        ...
        {/* Error Message */}
        {error && <div className="rounded-lg bg-red-500/20 border border-red-500/50 p-4 text-white">{error}</div>}
        ...
        {!isLoading && filteredProviders.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-white/70 max-w-md mx-auto">
              We couldn't find any service providers matching your search criteria. Try adjusting your filters or search
              terms.
            </p>
          </div>
        )}
